...
void workflowControls::skipRequestedSteps() const
{
    ...
    {
        const dictionary& workflowCtrls = meshDict.subDict("workflowControls");
		if( workflowCtrls.found("skipSteps") )
		{
				DynList<word> skipSteps = wordList(workflowCtrls.lookup("skipSteps"));

				if( mesh_.metaData().found("completedSteps") )
				{
					mesh_.metaData().set("completedSteps", skipSteps);
				}
				else
				{
					mesh_.metaData().add("completedSteps", skipSteps);
				}
		}
	}
...
bool workflowControls::restartRequested() const
{
    ...
    {
        const dictionary& workflowCtrls =
            meshDict.subDict("workflowControls");

        if( workflowCtrls.found("restartFromLatestStep") )
        {
            const bool restart =
                readBool(workflowCtrls.lookup("restartFromLatestStep"));
...
:
    ...
    isRestarted_(false),
	workflowSteps_(populateWorkflowSteps())
{
...
bool workflowControls::runCurrentStep(const word& stepName)
{
    ...
    {
        Info << "Step " << currentStep_ << " has already been executed, or skipped" << endl;
