...
#define  stvec  std::vector
#define  ststr  std::string
...
public:
   ...
   InputFile(const InputFile& input, const string& title)
	  ...
	  , informative(input.informative), multiline_(input.multiline_)
	{
		...
		renameKeys("stage1","processed stage1 , ..."); // EXP:;
		renameKeys("stage1_1","processed stage1_1 , ..."); // EXP:;
		#ifdef Dbg_SkipH
	...
	bool getData(std::istringstream& iss, const string& key, int importance=0) const
	{
		iss.clear();
	...
	void checkEndOfData(std::istringstream& iss, const string& key, const string message="", bool severe = true) const
