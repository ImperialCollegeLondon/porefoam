...
for iiCV=1:length(ihas)
	...
	%~ ViscCorrection=1.0+0.0*(1.0-0.5*(dPdzV1+dPczV1)./groupTime(volInt,VolI(Names,'Volume'))*dx/2 ...
	...
	viscEV2=viscEV-viscEV1-1.0e-12
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   222222222  EACH PHASE ENERGY LOSS 22222222 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	if (0)
		plot(t*10^3,( -(f_1./(abs(f_1)+1.0e-6)).*(viscEV1).*Volume./((Uz1Avg+1.0e-12).*AreaCross))./Pc_ref,Colours(1,:),'LineWidth',2,'markersize',2);
		hold on

		plot(t*10^3,( -(f_2./(abs(f_2)+1.0e-6)).*(viscEV2).*Volume./((Uz2Avg+1.0e-12).*AreaCross))./Pc_ref,Colours(3,:),'LineWidth',2,'markersize',2);
		...
		%plot(t*10^3,( -(f_1./(abs(f_1)+1.0e-6)).*(dpEd1+dpEc1).*Volume./((Uz1Avg+1.0e-12).*AreaCross))./Pc_ref,Colours(1,:),'LineWidth',1,'markersize',0);
		%plot(t*10^3,( -(f_2./(abs(f_2)+1.0e-6)).*(dpEd2+dpEc2).*Volume./((Uz2Avg+1.0e-12).*AreaCross))./Pc_ref,Colours(3,:),'LineWidth',1,'markersize',0);
...
%~
	...
	"RelPerm"
	[1.0-alpha1;   Uz1Avg.*Uz1Avg./( viscEV1) /(UzAvgSP*UzAvgSP/viscEVSP);   Uz2Avg.*Uz2Avg./( viscEV2) /(UzAvgSP*UzAvgSP/viscEVSP)]'

	semilogy(1.0-alpha1,Uz1Avg.*Uz1Avg./( viscEV1) /(UzAvgSP*UzAvgSP/viscEVSP),Colours(1,:),'LineWidth',2,'markersize',3);
	hold on
	semilogy(1.0-alpha1,Uz2Avg.*Uz2Avg./( viscEV2)   /(UzAvgSP*UzAvgSP/viscEVSP),Colours(3,:),'LineWidth',2,'markersize',3);
	semilogy(1.0-alpha1,t,Colours(2,:),'LineWidth',2,'markersize',0);

	disp('__________pppppppppppp______________-');


	dlmwrite(['relPerm', num2str(iCV), '.csv'], [1.0-alpha1;Uz1Avg.*Uz1Avg./( viscEV1)   /(UzAvgSP*UzAvgSP/viscEVSP) ...
	;Uz2Avg.*Uz2Avg./( viscEV2)   /(UzAvgSP*UzAvgSP/viscEVSP)]');


	%plot(1.0-alpha1,Uz1Avg.*Uz1Avg./( dpEc1+dpEd1)   /(UzAvgSP*UzAvgSP/dpEdVSP),Colours(1,:),'LineWidth',1,'markersize',0);
	%plot(1.0-alpha1,Uz2Avg.*Uz2Avg./( dpEc2+dpEd2)   /(UzAvgSP*UzAvgSP/dpEdVSP),Colours(3,:),'LineWidth',1,'markersize',0);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   333333333  REL PermNoPc %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	if (0)
		...
		plot(1.0-alpha1,Uz1Avg.*UzAvg./( viscEV)   /(UzAvgSP*UzAvgSP/viscEVSP),Colours(1,:),'LineWidth',2,'markersize',3);
		hold on
		plot(1.0-alpha1,Uz2Avg.*UzAvg./( viscEV)   /(UzAvgSP*UzAvgSP/viscEVSP),Colours(3,:),'LineWidth',2,'markersize',3);
