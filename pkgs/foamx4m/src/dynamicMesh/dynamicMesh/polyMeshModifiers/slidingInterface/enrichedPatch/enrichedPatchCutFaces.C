...
// Out of plane tolerace.  Used to reject points in the bent interfaces
const Foam::scalar Foam::enrichedPatch::outOfPlaneTol_ = 0.2;
...
//						 Pout << " newDir: " << newDir << " mag: " << magNewDir << flush;
						vector corrNewDir = newDir - normal*(normal & newDir);
						scalar magCorrNewDir = mag(corrNewDir);

						// Check how far out of plane is the point
						scalar outOfPlane =
...
//								 << abort(FatalError);
						...
						scalar curAtanTurn =
							atan2(corrNewDir & right, corrNewDir & ahead);

//						 Pout << " atan: " << curAtanTurn << endl;

						if
						(
							curAtanTurn > atanTurn
						 && outOfPlane < outOfPlaneTol_
						)
						{
...
//					 Pout<< "Double usage elsewhere, cannot be good" << endl;
				...
				if (mp[bestAtanPoint] == cutFaceGlobalPoints[0])
				{
					...
					if (debug)
					{
						Pout<< " local: " << cutFaceLocal
							<< " area: " << cutFaceLocal.mag(lp)
							<< " one side: " << faceI;
					}
...
void Foam::enrichedPatch::calcLocalCutFaces() const
{
	if (localCutFacesPtr_)
	{
		FatalErrorIn("void enrichedPatch::calcLocalCutFaces() const")
	...
	const faceList& cFaces = cutFaces();

	localCutFacesPtr_ = new faceList(cFaces.size());
	faceList& lcf = *localCutFacesPtr_;

	forAll (cFaces, faceI)
	{
		const face& cf = cFaces[faceI];

		face& curlcf = lcf[faceI];

		curlcf.setSize(cf.size());

		forAll (cf, pointI)
		{
			curlcf[pointI] = mpLookup.find(cf[pointI])();
		}
	}
}
...
void Foam::enrichedPatch::clearCutFaces()
{
	...
	deleteDemandDrivenData(localCutFacesPtr_);
}
...
const Foam::faceList& Foam::enrichedPatch::localCutFaces() const
{
	if (!localCutFacesPtr_)
	{
		calcLocalCutFaces();
